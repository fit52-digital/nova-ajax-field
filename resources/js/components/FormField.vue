	...
	methods: {
		findParentField() {
			...
			if (currentField.indexOf("__") > -1) {
				parentFieldTarget =
					currentField.substr(0, currentField.indexOf("__")) +
					"__" +
					parentFieldTarget;
			}
		...
		observeParentField() {
			...
			if (!targetInput) {
				console.warn(
					`Input field with id ${this.field.parent_field} not found`
				);
				return;
			}
		...
		inputSelected() {
			...
			if (Array.isArray(value)) {
				value.forEach((v) => {
					...
					const selectedOption = this.optionsCache.find(
